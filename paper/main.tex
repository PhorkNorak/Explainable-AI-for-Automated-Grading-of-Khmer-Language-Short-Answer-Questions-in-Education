...
(67\% exact integer-score match, 79\% within one point). (ii)
...
LLM is the clear winner ($67\%$ exact integer match vs.\ at most $57\%$ for the others, and the best
...
our deep cells are far tighter ($+1.4$ GTE, $+2.4$ Qwen 86.3/83.9) and even the simple classical
...
\begin{figure}[t]
\centering\includegraphics[width=0.78\columnwidth]{figures/fig_leakage.png}
...
LOO faithfulness on 135 test answers (seed 42). \emph{LOO occlusion is reliably faithful} for every
model (AOPC-comprehensiveness $+0.150$ classical, $+0.317$ BiLSTM): removing the words it flags moves
the score far more than random removal, and keeping only them preserves it (low sufficiency). LOO
requires no gradients and works identically on the non-differentiable SVR and the BiLSTM. (Encoder
and LLM faithfulness rows are HPC-pending.)
...
\midrule
Classical $\cdot$ occlusion & 0.125 & 0.030 & $+$0.096 & \textbf{0.150} & 0.011 \\
BiLSTM $\cdot$ occlusion     & 0.285 & 0.082 & $+$0.274 & \textbf{0.317} & 0.043 \\
\bottomrule
\end{tabular}
\\[2pt]
{\footnotesize Champion cells (refined cleaning). LOO occlusion is faithfully verified for both CPU-local families; encoder/LLM rows are HPC-pending.}
\end{table}

\begin{figure}[t]
\centering\includegraphics[width=0.82\columnwidth]{figures/fig_faithfulness.png}
\caption{AOPC comprehensiveness for LOO occlusion. Positive gap confirms the explainer beats random
removal for both CPU-local families.}
\label{fig:faith}
\end{figure}
