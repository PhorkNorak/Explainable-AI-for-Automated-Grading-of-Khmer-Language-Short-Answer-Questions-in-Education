...
clean    → strip invisibles + NFC + strip punctuation
segment  → clean + khmernltk.word_tokenize

Order applied in preprocess(): strip invisibles → NFC → strip punctuation →
(segment). "Invisibles" = Unicode format/control characters (zero-width space
U+200B, ZWNJ/ZWJ, BOM, soft hyphen, controls) plus bullet markers (• ◦ ‣ ▪ …) —
...
def preprocess(text: str, mode: str) -> str:
    ...
    if mode == "raw":
        return text
    # NFC (standard Unicode composition) + punctuation stripping.
    text = strip_punctuation(unicodedata.normalize("NFC", text))
    if mode == "clean":
        return text
    if mode == "segment":
